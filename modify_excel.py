...
def signal():
    ...
    path = time.strftime("%b_%Y_island") + ".xlsx"
    if not os.path.exists(path):
        # Get the newest XLSX file
        newest_xlsx_file = max(glob.glob("*2025_island.xlsx"), key=os.path.getctime)
    ...
    for wsrow in ws.iter_rows(min_row=2):
        for dfindex,dfrow in data.iterrows():
            if wsrow[0].value == dfrow['Stock']:
                ...
                wsrow[11].value = dfrow['Stop Loss']
...
def signal_public():
    time = datetime.now()
    prev_time = time - timedelta(days=1)
    path = time.strftime("%b_%Y") + "_island_signal.xlsx"
    if not os.path.exists(path):
        # Get the newest XLSX file
    ...
    for wsrow in ws.iter_rows(min_row=2):
        for dfindex,dfrow in data.iterrows():
            if wsrow[0].value == dfrow['Stock']:
                ...
                #wsrow[3].value = dfrow['Profit']
                #wsrow[4].value = dfrow['Profit %']
                wsrow[3].value = dfrow['Profit Factor']
                if (dfrow['BUY'] == 1):
                    wsrow[4].value = prev_time.strftime("%d/%m/%y")
                    wsrow[4].fill = PatternFill(start_color='FFFF00', end_color='FFFF00', fill_type='solid')
                    for i in range(5):
                        wsrow[i].fill = PatternFill(start_color='FFFF00', end_color='FFFF00', fill_type='solid')
                if (dfrow['SELL'] == 1):
                    wsrow[5].value = prev_time.strftime("%d/%m/%y")   
                    wsrow[5].fill = PatternFill(start_color='FFFF00', end_color='FFFF00', fill_type='solid') 
                    for i in range(5):
                        wsrow[i].fill = PatternFill(start_color='FFFF00', end_color='FFFF00', fill_type='solid')
                if (dfrow['CLOSE BUY'] == 1):
                    wsrow[6].value = prev_time.strftime("%d/%m/%y")
                ...
                if (dfrow['CLOSE SELL'] == 1):
                    ...
                    for i in range(5):
                        wsrow[i].fill = PatternFill(start_color='FFFF00', end_color='FFFF00', fill_type='solid')
                wsrow[8].value = dfrow['Last Close']
                wsrow[9].value = dfrow['Stop Loss']
